...
First version of this only handled `input()` at literal top-level code (inside loops/conditionals is fine, they don't introduce a scope) and bailed to the popup the moment `input()` appeared inside any `def`. That turned out to be the *common* case, not the rare one — a student's cows-and-bulls program wrapped its whole game loop in a function, which is normal, good practice. So the transform grew a real (if deliberately bounded) call-graph analysis: any top-level function that calls `input()` directly, or calls another such function — transitively, so `main()` → `get_number()` → `get_raw()` → `input()` all becomes one chain of generators — gets converted, and every *call site* of a converted function becomes `yield from` instead of a plain call.

The part that actually took the care: a blind name-based rewrite of call sites is only sound if there's no way a call could mean something other than "call this exact top-level function." So before rewriting anything, the scanner walks the whole tree checking for every way that could be false — the function referenced without being called (`f = ask`, or passed as `runner(ask)`), called on something other than a bare name (a method, an aliased variable), decorated, recursive (directly or in a cycle through other tainted functions), or with its name shadowed anywhere else in the file by a parameter, loop variable, or nested `def`. `input()` (or a reference to a tainted function) turning up inside a lambda, comprehension, class body, or a nested/closure function also forces a bail-out, since none of those get rewritten. Any one of these and the whole thing declines and falls straight back to `window.prompt()` — a needless popup is a fine failure mode; a tainted function silently handing back a generator object instead of its real return value, because some call site didn't get rewritten, is not. Turtle and p5 sketches are excluded outright regardless of any of this, since both rely on `setup()`/`draw()` (or user code) existing as real module-level globals afterward, which wrapping code in a generator function would break — those names would become local to the wrapper instead.
